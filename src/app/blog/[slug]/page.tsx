...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://royalkingseeds.au/blog/${slug}`,
    },
    openGraph: {
      title: post.title,
      description: post.metaDescription,
        ...
        <article>
          <header className="mb-8">
            <span className="text-[11px] text-[#192026]/40 uppercase tracking-[1px]">
              {new Date(post.publishDate).toLocaleDateString('en-AU', { month: 'long', day: 'numeric', year: 'numeric' })}
            </span>
          ...
          <div className="mt-10 bg-[#275C53] rounded-2xl p-8 text-center">
            ...
            <p className="text-white/70 text-sm mb-6 max-w-md mx-auto">
              Browse over 1,200 premium cannabis seeds with discreet shipping to all Australian states and territories and our 95% germination guarantee.
            </p>
            ...
            <div className="space-y-4">
              {recent.map((p) => (
                <Link key={p.slug} href={`/blog/${p.slug}`} className="block group">
                  <p className="text-[10px] text-[#192026]/40">{new Date(p.publishDate).toLocaleDateString('en-AU', { month: 'short', day: 'numeric' })}</p>
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            publisher: { "@type": "Organization", name: "Royal King Seeds", url: "https://royalkingseeds.au" },
            mainEntityOfPage: `https://royalkingseeds.au/blog/${slug}`,
          }),
        }}
      />
    </div>
