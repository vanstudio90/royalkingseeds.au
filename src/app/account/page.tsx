...
function getTrackingUrl(tracking: string): string | null {
  ...
  // Australia Post
  if (/^[A-Z]{2}\d{9}[A-Z]{2}$/.test(cleaned) || /^\d{13,22}$/.test(cleaned)) {
    return `https://auspost.com.au/mypost/track/#/details/${cleaned}`;
  }
  // UPS
                    ...
                    {orders.slice(0, 3).map(order => (
                      <div key={order.id} className="flex items-center justify-between py-2 border-b border-[#192026]/5 last:border-0">
                        <div>
                          ...
                          <span className="text-[11px] text-[#192026]/30 ml-2">{new Date(order.created_at).toLocaleDateString('en-AU')}</span>
                        </div>
                      ...
                      <div className="px-5 py-3 bg-[#F5F0EA] flex items-center justify-between flex-wrap gap-2">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <span className="text-[13px] text-[#192026]/60">{new Date(order.created_at).toLocaleDateString('en-AU')}</span>
                          </div>
                          <div>
                            <span className="text-[10px] uppercase tracking-[1px] text-[#192026]/30 block">Total</span>
                            <span className="text-[14px] font-bold text-[#275C53]">${order.total} AUD</span>
                          </div>
                        </div>
            ...
            <div>
              ...
              <div className="bg-white rounded-2xl border border-[#275C53]/5 p-6 space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div>
                    <label htmlFor="profile_state" className="text-[11px] font-semibold text-[#192026]/40 uppercase tracking-[0.5px] block mb-1">State/Territory</label>
                  ...
                  <div>
                    <label htmlFor="profile_zip" className="text-[11px] font-semibold text-[#192026]/40 uppercase tracking-[0.5px] block mb-1">Postcode</label>
                  ...
                  {reviews.map(review => (
                    <div key={review.id} className="bg-white rounded-2xl border border-[#275C53]/5 overflow-hidden">
                      <div className="px-5 py-3 bg-[#F5F0EA] flex items-center justify-between flex-wrap gap-2">
                        ...
                        <span className="text-[11px] text-[#192026]/30">{new Date(review.created_at).toLocaleDateString('en-AU')}</span>
                      </div>
                  ...
                  {uniqueStrains.map(strain => (
                    <Link key={strain.slug} href={`/${strain.slug}`} className="bg-white rounded-2xl border border-[#275C53]/5 p-5 hover:shadow-md hover:border-[#275C53]/15 transition-all group">
                      ...
                      <div className="text-[10px] text-[#192026]/30">
                        Purchased {new Date(strain.orderDate).toLocaleDateString('en-AU')}
                      </div>
                    </Link>
                  ))}
                </div>
                  ...
                  {wishlistItems.map((product: Product) => (
                    <div key={product.id} className="bg-white rounded-2xl border border-[#275C53]/5 p-4 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <span className="text-[13px] text-[#192026]/40 block">${product.price.toFixed(2)} AUD</span>
                      </div>
