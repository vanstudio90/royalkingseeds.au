...
export default function CheckoutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const orderNumber = `RKSAU-${Date.now().toString(36).toUpperCase()}`;
    ...
    <div className="max-w-[1100px] mx-auto px-4 sm:px-6 py-8 sm:py-12">
      {subtotal < FREE_SHIPPING_THRESHOLD && (
        <div className="bg-[#275C53]/10 text-[#275C53] text-[13px] rounded-xl p-3 mb-6 text-center">
          🇦🇺 Add <strong>${(FREE_SHIPPING_THRESHOLD - subtotal).toFixed(2)}</strong> more to unlock <strong>free shipping</strong> across Australia!
        </div>
      )}
      {subtotal >= FREE_SHIPPING_THRESHOLD && (
        <div className="bg-[#275C53] text-white text-[13px] rounded-xl p-3 mb-6 text-center">
          ✓ You qualify for <strong>free shipping</strong> across Australia!
        </div>
      )}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <input type="text" id="checkout_zip" name="zip" value={form.zip} onChange={e => updateForm('zip', e.target.value)} placeholder={country === 'AU' ? 'Postcode' : 'Postal code'} className="checkout-input" required />
                  </div>
                  {regions.length > 0 ? (
                    <div className="checkout-input !p-0 overflow-hidden">
                      ...
                      <select id="checkout_state" name="state" value={form.state} onChange={e => updateForm('state', e.target.value)} className="w-full h-full px-4 py-3 bg-transparent text-sm text-[#192026]/70" required>
                        <option value="">{country === 'AU' ? 'State/Territory' : 'Province'}</option>
                ...
                <label className={`flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${paymentMethod === 'credit-card' ? 'border-[#275C53] bg-white' : 'border-[#192026]/10 bg-[#F5F0EA]'}`}>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <span className="text-[14px] font-semibold text-[#192026]">Credit/Debit Card Payment (AUD)</span>
              ...
              {!form.billingSame && (
                <div className="space-y-3 mt-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <input type="text" value={form.billingZip} onChange={e => updateForm('billingZip', e.target.value)} placeholder={country === 'AU' ? 'Postcode' : 'Postal code'} className="checkout-input" required />
                  </div>
                </div>
              )}
            </section>
              ...
              <div className="border-t border-[#192026]/5 pt-4 space-y-2 text-[13px]">
                ...
                <div className="flex justify-between pt-3 border-t border-[#192026]/5 text-lg font-bold">
                  ...
                  <span className="text-[#275C53]">
                    ...
                    <span className="text-[11px] font-normal text-[#192026]/30">AUD</span>
                  </span>
                </div>
              </div>
            </div>
