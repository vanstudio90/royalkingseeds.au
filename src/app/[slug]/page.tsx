...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!product) {
    ...
    if (page) {
      return {
        ...
        alternates: { canonical: `https://royalkingseeds.au/${slug}` },
      };
    }
  ...
  return {
    ...
    openGraph: {
      title: `${product.name} ${type} Seeds — Royal King Seeds Australia`.trim(),
      description,
      type: 'website',
...
export default async function ProductPage({ params }: Props) {
  ...
  const productJsonLd = {
    ...
    url: `https://royalkingseeds.au/${product.slug}`,
  ...
  const breadcrumbJsonLd = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: 'https://royalkingseeds.au' },
      { '@type': 'ListItem', position: 2, name: categoryName, item: `https://royalkingseeds.au/product-category/${primaryCategory}` },
