...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: content?.h1 || `${category.name} — Buy Online Australia`,
    description: content?.intro?.slice(0, 160) || `Shop ${category.name} online with discreet Australia-wide shipping. Premium cannabis seed genetics from Royal King Seeds.`,
    alternates: {
      canonical: `https://royalkingseeds.au/product-category/${slug}`,
    },
  };
}
...
function filterProductsByCategory(products: any[], slug: string) {
  ...
  const filters: Record<string, (p: any) => boolean> = {
    ...
    'australian-premium-cannabis-seeds': (p) => p.categories.some((c: string) => c.toLowerCase().includes('australian') || c.toLowerCase().includes('premium')) || (parseFloat(p.thc_content || p.thcContent || '0') >= 20),
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            description: content?.intro || `Browse ${category.name} at Royal King Seeds Australia`,
            url: `https://royalkingseeds.au/product-category/${slug}`,
            mainEntity: {
              "@type": "ItemList",
              ...
              itemListElement: filtered.slice(0, 10).map((p, i) => ({
                "@type": "ListItem",
                position: i + 1,
                url: `https://royalkingseeds.au/${p.slug}`,
                name: p.name,
              })),
            },
          }),
