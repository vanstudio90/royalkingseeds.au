...
// Descriptions, short descriptions, meta tags are rewritten uniquely for Australian market
...
function generateMetaTitle(raw: any): string {
  ...
  return pick(raw.slug, [
    `${name} Seeds — Buy Online Australia`,
...
function generateMetaDescription(raw: any): string {
  ...
  return pick(raw.slug, [
    `Buy ${name} cannabis seeds online in Australia. ${type.charAt(0).toUpperCase() + type.slice(1)} · ${thc} THC · ${effects} effects. Discreet Australia-wide shipping. Germination guaranteed.`,
    `Shop ${name} seeds with fast, discreet Australian delivery. ${type.charAt(0).toUpperCase() + type.slice(1)} strain testing at ${thc} THC. ${effects}. Free shipping over $99.`,
    `${name} — premium ${type} cannabis seeds at ${thc} THC. ${effects} effects. Order online with discreet shipping across Australia. Germination guarantee included.`,
  ], 'mdesc');
}
...
function generateShortDescription(raw: any): string {
  ...
  const thc = raw.thc_content || '20%';
  const effects = (raw.effects || []).slice(0, 3).join(', ') || 'Relaxed, Happy';
  return `Buy ${name} cannabis seeds online in Australia. ${type.charAt(0).toUpperCase() + type.slice(1)} strain with ${thc} THC. Effects: ${effects}. Discreet Australia-wide shipping with germination guarantee.`;
}
