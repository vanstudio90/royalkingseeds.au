...
const FROM_EMAIL = 'Royal King Seeds <orders@royalkingseeds.au>';
const STORE_URL = 'https://royalkingseeds.au';
          ...
          <table width="100%" cellpadding="0" cellspacing="0">
            ...
            <tr><td style="text-align:center;">
              <div style="font-size:11px;color:rgba(255,255,255,0.6);line-height:1.8;">
                ...
                <a href="${STORE_URL}" style="color:${GOLD};text-decoration:none;">royalkingseeds.au</a>
              </div>
            </td></tr>
          </table>
        </td></tr>
...
function totalsBlock(order: OrderData) {
  ...
  rows += `<tr><td style="padding:10px 0 0;color:${DARK};font-size:20px;font-weight:800;">Total</td><td style="padding:10px 0 0;color:${GREEN};font-size:20px;font-weight:800;text-align:right;">$${order.total.toFixed(2)} AUD</td></tr>`;
...
function trackingBlock(trackingNumber: string) {
  const uspsUrl = `https://auspost.com.au/mypost/beta/track/details/${trackingNumber}`;
  return `
    <table width="100%" cellpadding="0" cellspacing="0" style="background:${BG};border-radius:6px;margin-bottom:24px;">
      <tr><td style="padding:20px;text-align:center;">
        ...
        <a href="${uspsUrl}" target="_blank" style="display:inline-block;background:${GREEN};color:#ffffff;font-size:14px;font-weight:700;padding:12px 28px;border-radius:6px;text-decoration:none;letter-spacing:0.5px;">Track on Australia Post</a>
      </td></tr>
    </table>`;
}
  ...
  return layout(
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background:${BG};border-radius:6px;margin-bottom:24px;">
      <tr><td style="padding:20px;text-align:center;">
        ...
        <div style="font-size:26px;font-weight:800;color:${DARK};">$${order.total.toFixed(2)} <span style="font-size:14px;color:#8a8078;">AUD</span></div>
...
export async function sendTrackingEmail(order: { order_number: string; customer_email: string; customer_name: string; tracking_number: string }) {
  ...
  const uspsUrl = `https://auspost.com.au/mypost/beta/track/details/${order.tracking_number}`;
  const html = layout(
    ...
    ${message("Click the button above or visit <a href='" + uspsUrl + "' style='color:" + GREEN + ";font-weight:600;'>Australia Post Tracking</a> to follow your package.")}
