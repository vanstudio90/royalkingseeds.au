...
// Localised for Australian growers — Southern Hemisphere seasons, metric units, Australian English
...
const P = { // Pillar article references — link instead of duplicate
  nutrient: (ctx: string) => `Nutrient uptake hinges on pH, medium type, and growth stage — factors that interact in ways most feeding charts fail to account for. Our ${a('complete nutrient guide', '/blog/cannabis-nutrient-deficiencies')} breaks down macro and micronutrient function, lockout patterns, and diagnostic methods in full detail.`,
  germ: (ctx: string) => `Germination success begins with viable seeds and the right method. Our ${a('germination guide', '/blog/cannabis-seed-identification-and-feminization')} covers the paper towel technique, timing, and transplanting steps that deliver the most consistent results for Australian growers.`,
  light: (ctx: string) => `Light spectrum, intensity, and schedule drive photosynthesis and flowering behaviour. Our ${a('grow light guide', '/blog/cannabis-growing-lights-and-phases')} covers LED vs HPS selection, PAR targets, and light-cycle management for each growth stage.`,
  pest: (ctx: string) => `Pest pressure varies by environment and season. Our ${a('pest management guide', '/blog/cannabis-pest-management')} identifies the most common threats to Australian growers and the IPM approaches that resolve them without damaging flower quality.`,
  vpd: (ctx: string) => `Temperature and humidity interact through VPD — vapour pressure deficit — which governs transpiration rate. Our ${a('VPD and humidity guide', '/blog/vpd-and-humidity-control-in-cannabis-cultivation')} explains the maths and the practical application for different grow setups.`,
  ...
  terp: (ctx: string) => `Terpenes shape flavour and modulate how cannabinoids feel — they are not just about smell. Our ${a('terpene guide', '/blog/pinene-and-terpenes-in-cannabis-and-aromatherapy')} covers the major compounds and their practical significance.`,
  ...
  anxiety: (ctx: string) => `Strain selection for anxiety is highly personal — what calms one person can overstimulate another. Our ${a('CBD and anxiety guide', '/blog/cbd-and-cannabis-for-anxiety-relief')} covers the cannabinoid and terpene combinations most consistently associated with calming effects.`,
  auto: (ctx: string) => `Autoflowers simplify growing by removing light-schedule management and finishing in 8-12 weeks. Our ${a('autoflower growing guide', '/blog/autoflower-cannabis-seeds-and-growing-guide')} covers the specific techniques that maximise yield from auto genetics.`,
};
...
// Localised for Australian growers — Southern Hemisphere, metric, Aussie English
// ════════════════════════════════════════════════════════════
...
function strainArticle(slug: string, title: string): string {
  ...
  return `
<p>${pick(slug, [
    `${name} is a ${type} that has earned a loyal following among Australian growers who value repeatable performance over strain-of-the-month hype. The genetics consistently test in the ${thc}% THC range with a ${terp}-dominant terpene profile, and the plant behaves predictably enough in the grow room that cultivators who run it once tend to keep it in rotation across seasons.`,
    `The marketing around ${name} tends to centre on potency and bag appeal. What actually makes this cultivar worth discussing is how it grows — the branching habit, the feeding tolerance, the flower density, and whether the final product matches what the genetics promise. That is what this profile covers for Aussie growers.`,
    `${name} occupies a useful space in the Australian cannabis market: a ${type} with enough potency to satisfy experienced consumers (${thc}% THC) and enough growing resilience to not punish intermediate cultivators for imperfect conditions. That combination is rarer than the number of strains on the market would suggest.`,
    `Most ${name} descriptions you will find online are rewrites of the same breeder summary. This profile takes a different approach — focused on what the plant actually does in an Australian grow room, what the terpene profile delivers after a proper cure, and who this strain genuinely suits versus who should look elsewhere.`,
  ], 'open')}</p>
...
<tr><td style="padding:6px 0;color:#192026aa"><strong>Indoor Yield</strong></td><td style="padding:6px 0">${yl} g/m² (competent setup, 200W+ LED)</td></tr>
...
<tr><td style="padding:6px 0;color:#192026aa"><strong>Climate</strong></td><td style="padding:6px 0">${type.includes('indica') ? 'Indoor preferred, outdoor in dry Australian climates (SA, WA)' : type.includes('sativa') ? 'Indoor with height management, outdoor in long-season subtropical zones (QLD, northern NSW)' : 'Performs well across most Australian growing environments'}</td></tr>
</table>
</div>
...
<p>${name} expresses as a ${type}. ${pick(slug, ['The growth structure tells you a lot about what to expect before the plant even enters flower', 'Understanding the architecture helps you plan training and space allocation from day one', 'The plant structure is where the genetic heritage shows most clearly'], 'struct')}: ${type.includes('indica') ? 'tight internodal spacing with broad fan leaves that create a naturally bushy canopy. Left untrained, the plant fills its allotted space laterally rather than stretching vertically, which suits growers working in height-limited tents or garages with low ceilings — a common setup in suburban Australia' : type.includes('sativa') ? 'wider node spacing and narrower leaf fingers that allow good light penetration through the canopy naturally. The vertical growth habit means height management is not optional in indoor spaces — plan for 40-60% stretch from flip to peak, and flip earlier than you would with a compact indica' : 'moderate internodal spacing that branches well after topping. The canopy develops with reasonable evenness, making it responsive to both LST and SCROG without demanding either. This is the kind of plant that rewards effort but does not require it to produce a decent result'}.</p>
<p>The genetic background influences more than structure. It shapes nutrient demand, stress tolerance, flowering speed, and terpene potential. Understanding the lineage helps predict how the plant will behave in your specific Australian environment — which is more useful than any marketing description.</p>

<h2>${pick(slug, ['How It Smells, Tastes, and Why Curing Matters', 'Terpene Character and What Develops in the Jar', 'Flavour and Aroma: From Fresh Cut to Cured Flower', 'The Nose on This Strain — And How to Get the Most from It'], 'h2b')}</h2>
<p>The primary terpene in most ${name} phenotypes is ${terp}, with ${terp2} as the secondary contributor. This combination produces ${terp === 'myrcene' ? 'an earthy, herbal base with a musky depth that darkens during cure. Fresh-cut flower smells herbaceous and slightly green. After 3-4 weeks in jars, the musk develops and the earthy character gains a richness that first-week cure cannot deliver' : terp === 'limonene' ? 'a bright citrus nose that is immediately identifiable when the jar opens. The flavour translates cleanly to vapour at 180-195°C — tangy, slightly sweet, with a clean finish that distinguishes it from the heavy earthiness of kush-leaning strains' : terp === 'caryophyllene' ? 'a peppery, dry spice character with woody undertones. This is not a "fruity" or "sweet" strain — the flavour profile reads as savoury and mature, which appeals to growers who have moved past the candy-genetics phase and appreciate depth over sweetness' : terp === 'pinene' ? 'a sharp, resinous aroma that reads as fresh pine with a clean, slightly medicinal quality. Pinene-dominant flower tends to feel "crisp" when vaped — less viscous on the palate than myrcene-heavy strains, with a brightness that lingers without heaviness' : terp === 'linalool' ? 'a soft floral quality with lavender undertones that smooth out any harshness. Linalool-forward flower is often described as "elegant" by users who care about terpene expression — it is a terpene that works best at moderate concentrations, adding refinement without dominating' : 'an herbaceous complexity with a slightly cedary, aromatic quality. Terpinolene-dominant strains are uncommon, which makes the flavour distinctive — users who find most cannabis monotonous in flavour often gravitate toward terpinolene cultivars for the novelty'}.</p>
<p>Curing matters enormously with ${name}. A one-week cure produces a flat, one-dimensional version of the terpene profile. Three to four weeks in glass jars at 60-62% humidity transforms the aromatic complexity — secondary terpenes emerge, harshness diminishes, and the flavour depth that the genetics are capable of actually shows up. Cutting cure short with this cultivar specifically is one of the most common reasons growers are underwhelmed by the final product. ${P.terp('')}</p>
...
<p>${pick(slug, [
    ...
    `Humidity management becomes critical during weeks 6-8 of flower. The bud structure is dense enough in most phenotypes that interior moisture gets trapped if airflow is not adequate. This is particularly relevant for Australian growers in humid coastal zones — Sydney, Brisbane, the Sunshine Coast — where ambient humidity can sit above 70% in late summer (February-March). Keep RH below 50% in late flower, ensure fans move air through the canopy (not just above it), and defoliate strategically in week 3 of flower to open up airflow paths.`,
    `The stretch phase after flipping to 12/12 runs longer than breeder descriptions suggest for most growers. Under LED, expect 30-40% height increase. Under HPS, closer to 50%. If you are growing in a 1.5 m tent, flip when plants reach 45-55 cm — not the 60 cm benchmark that works for more compact strains. Supercropping the tallest branches during early stretch is a viable emergency intervention if height gets away from you.`,
    `${name} responds aggressively to topping. A single top at the 4th node produces 4-6 strong lateral branches that develop into viable main colas without additional intervention. For growers who want maximum yield, combining a top at node 4 with light LST to spread the laterals flat produces an even canopy that fills a 1 m × 1 m space from a single plant with 4-5 weeks of veg time.`,
  ], 'grow')}</p>
<p>Indoor yield at competent levels runs ${yl} g/m² — that assumes 200W+ LED for a 1 m × 1 m area, appropriate nutrition, and reasonable environmental stability. First-time growers of this strain should expect the lower end. Growers with their environment dialled and 2+ grows of experience can target the upper range. Outdoor yield depends on season length, container volume, and direct sun hours — the long Australian growing season (October through April) pushes significantly higher in favourable climates. ${P.train('')}</p>
...
<h2>${pick(slug, ['Where This Strain Grows Best in Australia', 'Climate and Setup Considerations for Aussie Growers', 'Indoor, Outdoor, or Both? Environment Notes', 'Best Growing Conditions for This Cultivar in Australia'], 'h2e')}</h2>
<p>Indoor growers anywhere in Australia can run ${name} with standard equipment. The plant performs best at 22-27°C during lights-on and 17-21°C during lights-off — standard ranges that do not require specialised HVAC in most Australian homes. Relative humidity should taper from 55-60% in veg down to 45-50% in late flower.</p>
<p>Outdoor suitability depends on your climate zone. The ${flower}-week flowering period means you need frost-free conditions through at least mid-April for photoperiod versions. Growers in Queensland, northern NSW, coastal Western Australia, and South Australia have ample season length. Those in southern Victoria, Tasmania, or the ACT — where autumn frosts arrive earlier — should either grow indoors or choose autoflower versions. ${CL.outdoor} includes genetics evaluated for climate-specific performance across Australia's diverse growing zones.</p>
...
<p>${pick(slug, [
    ...
    `If you are chasing maximum THC above all else, our ${CL.thc} collection has genetics that test higher. ${name} is not about raw potency — it is about the total package: grow behaviour, terpene quality, effect character, and consistency across runs.`,
    `New growers in Australia looking for a forgiving cultivar that still delivers impressive results should consider ${name} seriously. It tolerates the temperature swings common in shed and garage grows during the Australian summer without punishing you for a 35°C day.`,
  ], 'who')}</p>
<p>Browse ${name} and related genetics in our ${CL.all}. All orders ship discreetly Australia-wide via Australia Post with tracking on every package.</p>

<h2>Frequently Asked Questions</h2>
<dl>
<dt>Can I grow ${name} outdoors in Melbourne or Sydney?</dt>
<dd>Yes. Both cities offer enough warm-season length (October-April) for the ${flower}-week flowering period. Melbourne growers should watch for late-autumn cold snaps in April and consider autoflower versions for extra margin. Sydney's milder coastal climate gives more flexibility with photoperiod timing.</dd>
<dt>What is the best time to start ${name} outdoors in Australia?</dt>
<dd>Germinate in September-October for transplant outdoors after the last frost risk passes. In subtropical Queensland, you can start as early as August. In cooler southern regions, mid-October is safer. Autoflower versions can be started anytime from October through January for a summer harvest.</dd>
<dt>How does ${name} handle Australian summer heat?</dt>
<dd>${type.includes('indica') ? 'Dense indica growth can struggle above 35°C — provide afternoon shade or shade cloth during heatwave conditions common in inland Australia.' : type.includes('sativa') ? 'The open canopy structure handles heat better than dense indicas. It performs well through typical Australian summers, but sustained 40°C+ days still warrant shade and extra watering.' : 'The balanced structure provides reasonable heat tolerance. In most Australian climates, standard summer conditions are well within its comfort zone. Provide shade cloth during extreme heatwave events above 38°C.'}</dd>
</dl>
`;
}
...
function pillarNutrient(slug: string, title: string): string {
  return `
<p>Most cannabis nutrient problems do not start in the bottle. They start in the root zone — with pH drift, salt buildup, overwatering, or a medium that stopped holding the right balance of air and moisture a fortnight ago. Growers who focus only on what they pour in miss the system that determines whether plants can actually use it. This guide covers the complete picture: what each nutrient does, how uptake actually works, why symptoms get misdiagnosed, and how to read your plants instead of following charts blindly.</p>

<h2>What Each Nutrient Does — And Why It Matters Specifically</h2>
<p><strong>Nitrogen (N):</strong> Drives vegetative growth — leaf expansion, stem elongation, chlorophyll production. A nitrogen-deficient plant yellows from the bottom up because nitrogen is mobile: the plant cannibalises older leaves to feed new growth. Excess nitrogen in flower produces dark, waxy foliage, delays maturation, and softens bud structure. Both conditions are common, and they look nothing alike.</p>
<p><strong>Phosphorus (P):</strong> Supports root development, energy transfer through ATP, and flower formation. Deficiency appears as dark purpling on stems and older leaves — often confused with cold stress or genetic colouration. The distinction: phosphorus deficiency purpling tends to appear on petioles and lower stems first and is accompanied by slow growth. Genetic purpling is typically uniform and not associated with growth reduction. Excess phosphorus locks out zinc and iron, producing interveinal chlorosis on new growth.</p>
...
<p><strong>Magnesium (Mg):</strong> Sits at the centre of every chlorophyll molecule. Deficiency shows as interveinal yellowing on older leaves — green veins with pale tissue between them. This is one of the most recognisable symptoms in cannabis and one of the most common in coco coir, which naturally binds magnesium and reduces its availability to roots.</p>
...
<p><strong>Coco coir:</strong> An inert medium with excellent air-to-water ratio that is nearly impossible to overwater. Coco is readily available in Australia from hydroponic shops and garden centres, making it a popular choice. But it has zero inherent nutrient content — you are feeding the plant directly at every watering. Mistakes show up within 2-3 days instead of the 7-10 day window soil provides. Coco also naturally exchanges calcium and magnesium ions, which means coco-specific nutrient lines include extra cal-mag, and using soil nutrients in coco frequently produces cal-mag deficiency. Feed at every watering, monitor runoff EC, and never let coco dry out completely.</p>
<p><strong>Hydroponics:</strong> Direct nutrient delivery in solution. Growth rates 20-30% faster than soil. Yields higher with the same genetics and light. But pH and EC must be monitored daily — sometimes twice in warm conditions, which is particularly relevant during Australia's hot summers when nutrient solution temperatures can climb rapidly. Equipment failures (pump dies, timer glitches, air stone clogs) can damage or kill a plant in hours, not days. Hydro is not harder than other methods — it is less tolerant of inattention and inconsistency.</p>

<h2>Reading Your Plants Instead of Following Charts</h2>
<p>Feed charts from nutrient manufacturers are starting points calibrated for average conditions. Your water's mineral content (Australian tap water varies enormously — Perth is relatively soft, Adelaide and Melbourne can be harder), your medium's buffering capacity, your light intensity, your temperature and humidity, your pot size, and your genetics all change how much the plant needs. The chart cannot account for all of these variables. Your plants can.</p>
<p>Learn to read patterns across the whole canopy instead of reacting to individual leaves. A single yellowing leaf on a lower branch is normal canopy ageing — the plant drops leaves that no longer receive adequate light. A wave of yellowing moving upward from the bottom signals nitrogen depletion. Burnt tips appearing on upper new growth during weeks 4-6 of flower suggests PK is too aggressive. Pale new growth with green veins points to iron lockout from high pH. Twisted, spotted new growth suggests calcium deficiency.</p>
...
<p><strong>Peak flower (weeks 3-6):</strong> Maximum PK demand. The plant is building bud mass, producing trichomes, and synthesising terpenes and cannabinoids. This is when potassium deficiency most commonly appears in programs that do not adjust for the demand increase.</p>
<p><strong>Late flower / flush (final 1-2 weeks):</strong> Reducing or eliminating nutrients allows the plant to metabolise stored salts, which many growers believe improves smoothness and flavour. Whether flushing is scientifically validated is debated, but the practice reduces salt content in the finished flower and costs nothing.</p>
...
<dd>The scientific evidence for flushing is debated, but it is a common practice that does not harm the plant. Reducing feed in the final 7-14 days allows the plant to use stored nutrients, potentially improving flavour smoothness. At minimum, it reduces your nutrient costs for those final waterings.</dd>
...
<p>Germination is where every grow begins and where a surprising number of grows fail before they start. The method matters less than the conditions — warmth, moisture, darkness, and patience — but understanding why seeds fail and how to handle the fragile taproot stage separates growers who lose 30% of their seeds from growers who lose almost none. For Australian growers, spring (September-October) is the ideal time to germinate if you are planning an outdoor grow.</p>

<h2>The Paper Towel Method: Why It Works Best for Most Growers</h2>
<p>Place seeds between two sheets of damp (not dripping) paper towel on a plate. Cover with a second plate to create a dark, humid chamber. Store at 22-26°C — on top of a modem, set-top box, or seed heat mat. Check every 12 hours. Re-dampen the towel if it is drying — the seed must stay moist but not submerged. When the taproot reaches 1-2.5 cm, transplant root-down into your medium at about 1 cm depth.</p>
<p>Why this method outperforms direct-to-soil or water soaking for most Australian growers: you can see exactly when the seed cracks and monitor root development without disturbing the medium. Direct-to-soil works but gives you no visibility into whether the seed is progressing or dead. Water glass soaking works for the initial crack but seeds left submerged more than 24 hours risk drowning.</p>

<h2>Why Seeds Fail to Germinate</h2>
<p><strong>Old or improperly stored seeds:</strong> Viability drops with age, heat exposure, and light. Seeds stored in a warm drawer during an Australian summer may have 30-40% germination rates. Fresh seeds from climate-controlled storage typically germinate above 90%. Storage is the single largest factor in germination success — keep seeds in the fridge, not in a hot shed.</p>
...
<p><strong>Cold temperatures:</strong> Below 18°C, the enzymatic processes that crack the seed shell slow dramatically. Below 15°C, they essentially stop. This is relevant for Australian growers germinating in late winter — your house may feel warm enough to you, but unheated rooms in Melbourne, Canberra, or Hobart can dip below optimal germination temperature overnight.</p>
...
<p>The transition from paper towel to growing medium is a vulnerability point. Pre-moisten your medium before transplanting — a dry medium wicks moisture away from the fragile root. Make a small hole about 1 cm deep. Place the seed root-down. If the root curves, orient it so the tip points generally downward — it will self-correct. Cover with loose medium. Do not pack it. Water gently around (not directly on) the seed.</p>
<p>Light should be introduced once cotyledon leaves emerge and open — typically 2-4 days after planting. Start with low intensity (200-300 PPFD or fluorescent at 30+ cm). Seedlings that stretch tall and thin are reaching for inadequate light — lower the light or increase intensity gradually.</p>

<h2>Seed Viability, Quality, and Storage</h2>
<p>Fresh seeds from a reputable source with climate-controlled storage germinate faster and more reliably than aged seeds from uncertain storage conditions. When buying, prioritise freshness and source quality over strain novelty. Seeds stored properly (sealed container, refrigerator, 4-10°C, low humidity) remain viable for 2-5 years. Seeds left in room-temperature conditions — especially in Australia's warm climate — degrade within 6-12 months. Dark-coloured, firm seeds with intact shells generally indicate maturity and viability. Pale, soft, or cracked seeds are more likely to fail.</p>

<h2>Frequently Asked Questions</h2>
<dl>
<dt>How long should germination take?</dt>
<dd>Most viable seeds crack within 24-72 hours in proper conditions. Some take up to 7 days, especially older seeds. If nothing has happened after 7 days at consistent 22-26°C with proper moisture, the seed is likely not viable.</dd>
<dt>Can I germinate seeds directly in soil?</dt>
<dd>Yes, but you cannot monitor progress. Plant 1 cm deep in moist medium, keep warm, and wait. If nothing emerges after 10-14 days, the seed likely failed. The paper towel method is preferred because it gives you visibility into the process.</dd>
...
<p>Optimal drying conditions: 15°C, 60% relative humidity, complete darkness, gentle air circulation that moves air through the room without blowing directly on the hanging flower. These conditions produce a 10-14 day dry — slow enough for chlorophyll to break down fully, water activity to equalise from dense bud interiors through stems, and harsh volatile compounds to off-gas.</p>
<p>Fast drying (3-5 days in warm, dry conditions above 21°C) produces hay smell, harsh smoke, and muted terpenes — regardless of how well the plant was grown. This is a particular risk for Australian growers harvesting outdoor crops in late March or April, when ambient temperatures can still sit above 25°C. The hay smell specifically comes from chlorophyll that has not had time to decompose. There is no cure-stage fix for a bad dry — you can improve a fast-dried flower somewhat with extended curing, but you cannot recover what was destroyed by heat and speed. Consider drying in an air-conditioned room if your ambient conditions are too warm.</p>
<p>The stem snap test is imprecise but serviceable for determining when to trim and jar: when smaller stems snap cleanly rather than bending, the exterior is dry enough. But dense bud interiors will still hold moisture, which is why the cure stage — an equalisation process — is essential.</p>

<h2>Curing: Building the Other 30-40% of Quality</h2>
<p>Curing is controlled moisture equalisation inside sealed glass jars. Place trimmed flower in mason jars filled about 75% capacity with a small hygrometer in each jar. Target humidity: 60-62% RH inside the jar. Open jars briefly (30-60 seconds) once or twice daily for the first 7 days to exchange air and release excess moisture. After the first week, reduce to once daily, then every 2-3 days for an additional 2-4 weeks.</p>
<p>During this time, several things happen: remaining chlorophyll continues to break down (reducing harshness), terpene profiles mature and develop depth (secondary aromatics emerge that were not present at the start), starch and sugar conversion improves the smoothness of the smoke, and overall flavour complexity increases. Minimum viable cure: 2 weeks. Noticeable improvement continues through 4-6 weeks. Some cultivators cure dense ${CL.kush} genetics for 8+ weeks and report continued improvement in smoothness.</p>
...
<p>Once the cure is complete, long-term storage priorities are: darkness, cool temperature (15-21°C), stable humidity (58-62%), and minimal air exposure. Glass jars remain ideal. Vacuum sealing works for long-term storage (months) but flattens buds. Avoid plastic bags (static charges strip trichomes), avoid frequent opening (terpene evaporation), and avoid heat and light (THC degrades to CBN, terpenes evaporate). Properly stored, cured flower maintains quality for 6-12 months. In Australia's warmer climate, storing jars in a cupboard away from heat sources — not in a hot garage or shed — is essential.</p>

<h2>Frequently Asked Questions</h2>
<dl>
<dt>Can I use the pistil colour method instead of trichome checking?</dt>
<dd>Pistil colour (white to orange/brown) gives a rough estimate but is unreliable as a sole indicator. Some strains change pistil colour early while trichomes are still clear. Others maintain white pistils when trichomes are significantly amber. Use pistils as a general signal but confirm with magnified trichome observation.</dd>
...
<dt>How do I dry in a humid Australian climate?</dt>
<dd>In environments where ambient humidity exceeds 65% — common along the east coast of Australia during the autumn harvest period — a dehumidifier in the drying space is necessary. Without it, drying takes too long and mould risk increases significantly. Target 55-60% in the drying room. Air movement helps but should not blow directly on flower.</dd>
<dt>Is burping jars during cure really necessary?</dt>
<dd>Yes, during the first week. Fresh-jarred flower releases moisture from the interior of dense buds, and without air exchange, humidity inside the jar climbs above 65% and creates mould conditions. After the first week, when interior humidity stabilises, burping frequency can decrease.</dd>
</dl>
`;

  // Generic pillar for other topics — still deep and unique
  return `
<p>${topicLower.charAt(0).toUpperCase() + topicLower.slice(1)} is one of those cultivation fundamentals where surface-level understanding produces surface-level results. The difference between growers who consistently produce quality flower and growers who struggle with the same problems every run often traces back to how well they understand and manage this specific area of growing. This guide goes deeper than the typical overview — covering the mechanism, the stage-by-stage application, the common misunderstandings, and the decision points that experienced Australian growers navigate intuitively.</p>

<h2>Why This Matters More Than Most Growers Initially Realise</h2>
<p>${pick(slug, [
...
<p>Understanding this fundamental tradeoff — growth versus survival — explains why ${topicLower} problems reduce yield even when they do not kill the plant. A cannabis plant surviving sub-optimal conditions is channelling energy into stress management that would otherwise go toward flower production, trichome development, and terpene synthesis.</p>
...
<p><strong>Following static rules instead of reading plant response:</strong> "Keep ${topicLower} at X" is a starting point, not a law. Your specific genetics, medium, pot size, light intensity, and ambient conditions all modify the optimal range. Plants tell you what they need through leaf position, colour, growth rate, and turgor pressure. Learning to read those signals is more valuable than memorising numbers.</p>
...
<dd>Depending on severity, 2-7 days. Extreme conditions produce visible stress within hours (wilting, leaf curling). Moderate mismanagement takes several days to show as growth slowdown or colour changes. By the time symptoms are obvious, the underlying issue has been present for a while.</dd>
...
<dd>Yes. Genetics adapted to specific environments handle related conditions better. Tropical sativa genetics handle heat and humidity — useful for Queensland and northern NSW growers. Mountain indica genetics handle cold and UV. Autoflowers bred from ruderalis genetics tolerate wider ranges overall. Matching genetics to your conditions is one of the highest-impact decisions you can make.</dd>
</dl>
`;
}
...
<p>${pick(slug, [
    ...
    `Most growers encounter ${topic.toLowerCase()} issues after their first grow or two — once they are past the basics and starting to notice nuances in plant behaviour that they missed as beginners. This is the point where focused topic knowledge pays off.`,
  ], 'open')}</p>
...
<p>${pick(slug, [
    `Before making any adjustment, gather three data points: pH of your input water and runoff, EC of your runoff compared to your input, and a clear photograph of the affected growth under white light (not HPS or blurple). pH tells you whether the issue is a lockout. EC comparison tells you whether salts are accumulating. The photo — taken under neutral lighting — shows the true colour and pattern of the symptom without distortion from grow lighting.`,
    `The diagnostic approach for ${topic.toLowerCase()} starts with ruling out environmental causes that mimic nutrient symptoms. Heat stress, light stress, and root-zone problems all produce leaf changes that look like nutrient issues. If your environment is within normal ranges (21-29°C, 45-60% RH, no light burn on upper canopy), then the symptom is likely nutritional. From there, leaf position (upper vs lower canopy) and symptom type (discolouration, necrosis, deformation) narrow the field.`,
    `One reliable diagnostic trick for ${topic.toLowerCase()}: observe whether the issue is progressing or stable. A deficiency that is getting worse daily despite normal feeding points to lockout — the nutrient is present but unavailable. A deficiency that stabilised after your last feed adjustment suggests the correction is working and the plant needs time to show recovery on new growth (old damaged tissue does not heal).`,
  ], 'diag')}</p>
...
<p>${pick(slug, [
    ...
    `The most common real-world trigger we hear about from Australian growers for ${topic.toLowerCase()}: running the same feed schedule from early veg through late flower without adjusting for the plant's changing needs. What works in week 2 of veg is not what the plant needs in week 5 of flower, and the symptoms that emerge from this mismatch often look like ${topic.toLowerCase()} problems specifically.`,
    `Environmental interactions that trigger ${topic.toLowerCase()} issues: high-intensity LED lighting increasing calcium demand beyond what the feed provides, cold root-zone temperatures during an Australian winter reducing uptake efficiency, and overwatering reducing oxygen in the root zone and impairing the active nutrient transport that roots require.`,
  ], 'trigger')}</p>
...
<p>Small pot → 4 litre → 12-20 litre final container is the standard progression. Each step roughly triples the root space. Jumping from a small pot directly to a 20-litre pot works but increases the risk of overwatering — the excess unused medium stays wet and creates anaerobic zones while the small root ball occupies only a fraction of the container. Gradual up-potting keeps the root-to-medium ratio manageable at each stage.</p>
<p>For ${CL.auto}, skip the progression entirely — plant directly into the final container (12-20 litres) because autoflowers lose precious time recovering from transplant shock. Their short lifecycle means every day of stalled growth directly reduces final yield. Starting in the final pot avoids this risk.</p>
...
<p>Cloning preserves the exact genetic expression of a specific plant — the same terpene profile, growth habit, potency, and phenotype, replicated identically. If you have found a phenotype you love during a seed run, cloning lets you keep it indefinitely. The tradeoff: clones carry the mother's biological age (they do not "reset" like seeds), do not develop taproots (which limits outdoor vigour), and require maintaining a mother plant or access to a reliable clone source.</p>
<p>Starting from ${CL.fem} gives you genetic diversity (every seed is a slightly different phenotype), taproot development, fresh vigour, and no dependency on maintaining a mother. Most home growers in Australia benefit from starting with seeds to explore a strain's phenotype range, identifying their favourite, and then cloning that specific plant for subsequent runs.</p>
<h2>Taking Cuttings: Technique That Determines Success Rate</h2>
<p>Select a branch tip 10-15 cm long from the lower-middle section of a healthy, non-stressed mother in vegetative state. Cut at a 45-degree angle just below a node using a clean, sharp razor blade — not scissors, which crush the stem and reduce uptake. Remove lower leaves immediately, leaving 2-3 sets of upper leaves. Trim the remaining leaf tips by 50% to reduce transpiration demand while the cutting has no roots.</p>
<p>Dip the cut end in rooting hormone — gel formulations maintain contact better than powder. Insert into pre-moistened rooting medium (rockwool cubes, peat plugs, or rapid rooters). Place inside a humidity dome at 24-27°C with 80-90% RH. Roots appear in 7-14 days. Light should be gentle — a T5 fluorescent or low-intensity LED. Too much light drives photosynthesis that the rootless cutting cannot support.</p>
...
<p><strong>Low humidity:</strong> Without roots, the cutting relies on foliar moisture absorption and reduced transpiration. Below 70% RH, it dries out faster than it can hydrate. In dry Australian climates (Adelaide, inland areas), a humidity dome is essential.</p>
...
<p>Coco holds an excellent air-to-water ratio that makes overwatering nearly impossible — a significant advantage for beginners who tend to water too frequently. Coco coir is widely available across Australia from hydroponic retailers and garden centres. But coco has zero inherent nutrient content. You are feeding the plant directly with every watering, and the medium provides no buffer if your feed is wrong. Problems that take 7-10 days to appear in soil show up in 2-3 days in coco.</p>
<p>The critical coco-specific issue: cation exchange. Coco fibre naturally binds calcium and magnesium, reducing their availability to roots. This is why coco-specific nutrient lines include extra cal-mag, and using soil nutrients in coco reliably produces cal-mag deficiency within 2-3 weeks. If you grow in coco, use coco-formulated nutrients or supplement separately.</p>
...
<p>Hydro delivers nutrients directly to roots suspended in oxygenated solution. Growth rates are 20-30% faster than soil. Yields are proportionally higher with the same genetics. But pH and EC drift must be monitored daily — sometimes twice in warm conditions, and Australian summers can push nutrient solution temperatures dangerously high. Reservoir chillers or frozen water bottles may be needed when ambient temperatures exceed 30°C. Equipment failures are emergencies in hydro: if an air pump dies, roots lose oxygen within hours. If a water pump fails, plants dry out rapidly.</p>
...
<dt>Why do my soil plants grow slower than my mate's coco plants?</dt>
...
<p>Water thoroughly when the medium is dry at 2-5 cm depth (finger test) or when the container feels noticeably lighter than after the last watering. Apply water slowly and evenly until 10-20% runs out the bottom — this ensures the entire root zone is saturated and flushes minor salt accumulation. Then do not water again until the medium meets the dryness criteria. In soil, this cycle typically takes 2-4 days depending on pot size, temperature, humidity, and plant size. In coco, it is usually daily or every other day because coco dries faster. During hot Australian summers, outdoor plants may need watering twice daily.</p>
...
<p><strong>Curling upward (taco-ing, leaf edges folding up):</strong> Heat stress or light stress. The leaf reduces its surface area exposed to intense light or heat. This is particularly common for Australian growers during summer heatwaves when daytime temperatures exceed 35°C. Fix: raise the light 5-10 cm, increase airflow between the canopy and the light, or reduce ambient temperature. ${P.vpd('')}</p>
...
<p>Small pots dry out faster, requiring more frequent watering but also providing faster wet-dry cycles that roots prefer. Large pots hold moisture longer, which benefits established plants but creates overwatering risk for small plants that have not yet colonised the full volume. A seedling in a 20-litre pot sits in constantly moist medium around its small root ball — exactly the conditions that promote root problems.</p>
<p>The practical solution: start in small containers and up-pot as the plant grows. Or, for ${CL.auto} that go directly into final containers, water in a small circle around the base and gradually expand the watering radius as roots colonise outward.</p>
<h2>Frequently Asked Questions</h2>
<dl>
...
<p>Yield is the product of four interacting factors: genetics (the ceiling), light energy (the primary input), root health (the delivery system), and canopy management (the efficiency multiplier). Maximising one while neglecting others produces diminishing returns. A 600W LED cannot compensate for rootbound plants in 4-litre pots. Perfect nutrients cannot overcome a canopy where only the top 15 cm receives adequate light.</p>
<h2>The Genetics Factor: Setting the Ceiling</h2>
<p>Strain selection sets the upper boundary of what is possible. ${CL.fem} from high-yield lineages — Big Bud, Critical Mass, Blue Dream, Gorilla Glue — have genetic capacity for larger harvests than compact autoflowers at the same light level. But that potential only materialises with adequate veg time, sufficient container volume (12-20 litres minimum for photoperiod plants aiming for yield), and training to distribute bud sites across the canopy.</p>
<p>Growing a high-yield strain in a 4-litre pot with 2 weeks of veg wastes the genetic advantage — you could achieve similar results with a less demanding cultivar. Match your ambition to your setup: if space and time are limited, ${CL.auto} offer respectable yields with less input. If you can give a plant 4-6 weeks of veg in a 20-litre container, photoperiod genetics reward that investment.</p>
<h2>Light: The Primary Yield Input</h2>
<p>More light (up to the plant's saturation point) means more photosynthesis means more growth. The relationship is roughly linear from 200 PPFD to 800-1000 PPFD for most cannabis strains. Beyond 1000 PPFD, CO2 supplementation is needed to prevent the plant from light-saturating and wasting energy as heat. For a 1.2 m × 1.2 m tent, 400-600W of quality LED covers the canopy at adequate intensity for strong flower production. For a 1 m × 1 m area, 200-350W is the practical range. ${P.light('')}</p>
<p>Light distribution matters as much as total wattage. A single point-source light creates a hot spot in the centre and dim edges. Multi-bar LED fixtures or well-spaced arrays produce more even coverage, which means more uniform bud development across the entire canopy rather than one heavy main cola surrounded by underdeveloped larf.</p>
<h2>Canopy Management: The Yield Multiplier</h2>
<p>An untrained cannabis plant develops one dominant main cola and diminishing side branches in a "Christmas tree" shape. Only the top 30-45 cm receives enough light for dense bud production. Everything below is shaded larf — airy, underdeveloped flower that is not worth the plant's energy.</p>
...
<p><strong>1 m × 1 m tent, 200W LED, autoflower:</strong> 60-150 g per plant, 120-250 g total from 2-4 plants. This is the most accessible setup and produces enough for personal use.</p>
<p><strong>1.2 m × 1.2 m tent, 400W LED, feminised photoperiod, 4-5 week veg:</strong> 120-250 g per plant, 350-700 g total from 4 plants with training. This is where home growing starts to feel genuinely productive.</p>
<p><strong>1.2 m × 1.2 m tent, 600W LED, SCROG, single plant, 6-8 week veg:</strong> 250-500 g from one plant that fills the entire canopy. This maximises the genetics' potential through extended veg and aggressive training.</p>
<p><strong>Outdoor, full sun, 60+ litre container, 6-month Australian growing season (Oct-Apr):</strong> 250-700 g per plant depending on genetics, climate zone, and growing skill. Queensland and northern NSW growers with long subtropical seasons report the upper range consistently.</p>
...
<dd>0.5-1.0 grams per watt of LED is a reasonable range for intermediate growers. Experienced growers with dialled environments and training can exceed 1 g/watt. Beginners typically land at 0.3-0.5 g/watt on their first grow.</dd>
<dt>Do autoflowers yield less than photoperiod strains?</dt>
<dd>Generally yes — autoflowers produce 60-150 g per plant versus 120-250+ g for photoperiods. But autoflowers finish faster and can produce 3-4 harvests per year in the same space, which can equal or exceed the annual yield of fewer but larger photoperiod runs.</dd>
<dt>Does CO2 supplementation increase yield?</dt>
<dd>Only if your light intensity exceeds 800-1000 PPFD and your other variables (temperature, humidity, nutrients) are optimised. Adding CO2 to a setup with inadequate light produces no benefit. It is an advanced technique that only makes sense after all other factors are dialled in.</dd>
</dl>`;
...
<p>Pre-flowers appear at the nodes (where branches meet the main stem) during late veg or early flower — typically 4-6 weeks from seed. Male pre-flowers are small, round, smooth balls on short stalks. Female pre-flowers are tear-drop shaped with white pistil hairs (stigmas) emerging from the tip. The distinction is subtle at first and requires close inspection. A jeweller's loupe helps. Check the nodes of the 4th-6th branch — these tend to show sex first.</p>
<p>If growing from regular (non-feminised) seeds, check for sex daily starting at week 4. Males should be removed from the grow space before any pollen sacs open — a single open sac can pollinate an entire room, seeding every female flower and dramatically reducing flower quality and yield.</p>
...
<p><strong>Extreme temperature swings:</strong> Nighttime drops below 13°C or daytime spikes above 32°C sustained over multiple days. Occasional brief fluctuations are tolerable; sustained extremes are not. This is particularly relevant for Australian growers in inland regions where day-night temperature differentials can be dramatic.</p>
...
<p><strong>Genetic predisposition:</strong> Some strains are inherently more prone to hermaphroditism than others. This is partly why choosing stable genetics from reputable breeders matters — poorly bred or unstable genetics herm under conditions that would not trigger it in well-stabilised lines.</p>
...
<h2>Frequently Asked Questions</h2>
<dl>
<dt>Will feminised seeds hermaphrodite more than regular seeds?</dt>
<dd>Not if they are well-bred. High-quality feminised seeds from reputable breeders have been selected for genetic stability across multiple generations. Poorly made feminised seeds from unstable stock may herm more readily. Source quality matters.</dd>
...
<p>The common assumption about ${topicLower} is that it follows the same rules across all setups. In practice, what works in a 1.2 m × 1.2 m tent with coco and LED diverges significantly from what works in a soil grow under HPS, or an outdoor grow in humid coastal Queensland. The variable that most guides treat as universal is actually setup-dependent — and the growers who figure this out stop chasing generic solutions and start adapting to what their specific environment demands.</p>
...
<dd>Usually because your conditions differ from the conditions where the advice you are following was developed. Altitude, water mineral content (which varies widely across Australian cities), ambient temperature, humidity, and even electrical supply consistency all modify how ${topicLower} management plays out. Calibrate advice to your actual environment.</dd>
...
<p>Cannabis produces a constant stream of visual signals — leaf colour variations, slight drooping, growth rate changes, stem flexibility. Most of these are normal plant behaviour, not indicators of problems. Learning to distinguish signal (actual problems requiring intervention) from noise (normal variation) is the skill that separates calm, effective growers from anxious overcorrectors.</p>
<p>Signals worth acting on: progressive yellowing moving in a consistent direction (up or down the canopy), tip burn on new growth, wilting that does not recover during lights-off, and sudden growth stall after weeks of steady development. Noise to ignore: individual leaf yellowing on lower branches, slight afternoon droop in warm conditions (common during Australian summer), and minor colour variation between leaves at different canopy positions.</p>
...
<p>The growers who handle ${topicLower} best are not following more complicated rules. They have run enough grows to recognise patterns that newer growers have not seen yet. Each grow adds to your database of observations — what stress looks like in your specific setup, how quickly your specific genetics respond to corrections, and which of the many possible interventions actually matters in your conditions.</p>
...
<p>${topic} involves a fundamental trade-off that generic guides skip: optimising ${topicLower} for one outcome (maximum yield, for example) often conflicts with optimising for another (terpene preservation, flower density, or harvest timing flexibility). Understanding which trade-off you are making — and which outcome you actually care about most — clarifies decisions that otherwise feel arbitrary.</p>
<p>Most home growers should optimise for reliability over peak performance. A consistent approach that produces 80% of theoretical maximum every run beats an aggressive approach that produces 100% once and fails the next three times.</p>
...
<p>The best indicator: is the plant producing new healthy growth at a consistent rate? Healthy new growth means the system is working. Slow, stunted, or discoloured new growth means something is off — even if your readings look correct.</p>
...
<p>After correcting the underlying issue, recovery shows in new growth — not old damaged tissue. Existing damaged leaves will not heal. What you watch for: new leaves emerging clean and healthy-coloured within 7-14 days of correction. If new growth continues to show symptoms, either the diagnosis was wrong or the correction was insufficient.</p>
...
<dd>A $30 AUD thermometer/hygrometer with logging capability and a $20 AUD pH pen provide 90% of the diagnostic data you need. Expensive monitoring systems add convenience and remote access but do not replace the fundamental habit of checking your plants regularly with your own eyes.</dd>
<dt>My last grow had no ${topicLower} issues but this one does. Same setup, same strain. Why?</dt>
<dd>Environmental conditions change between grows — seasonal temperature and humidity shifts (significant in Australia between summer and winter grows), water source mineral content variation, ageing equipment performance, and even different phenotypes within the same strain can produce different responses to identical management. Each grow is its own system, even if the inputs look the same.</dd>
</dl>`;
    }
  }
...
function environmentArticle(slug: string, title: string): string {
  ...
  if (isDesert) return `
<p>Growing in Australia's arid and semi-arid zones — from inland South Australia through western NSW and across much of Western Australia and the Northern Territory — breaks the usual rules. The assumption that more sun equals more yield falls apart above 35°C, where photosynthesis efficiency drops, stomata close, and the plant enters survival mode. Dry-climate cannabis cultivation is fundamentally about managing the gap between what intense sunlight could produce and what heat stress allows — and the growers who bridge that gap do it through irrigation strategy, root-zone management, microclimate engineering, and cultivar selection that most generic grow guides never address.</p>

<h2>What Happens to Cannabis Above 30°C</h2>
<p>Between 30-35°C, photosynthesis efficiency begins declining as enzymatic processes slow. Above 35°C, it drops substantially — the plant closes stomata to conserve water, which simultaneously shuts down CO2 intake and halts growth. Meanwhile, transpiration demand spikes. A plant in 38°C dry air drinks 2-3x what the same genetics need at 24°C. This creates a compounding problem: the plant needs more water to transpire, but it has shut down the gas exchange that allows productive growth. It is spending energy on survival instead of flower production.</p>
<p>Root-zone temperature compounds the aboveground stress. In containers sitting in direct Australian sun — especially dark-coloured plastic pots — root-zone temperatures can exceed 32°C, which damages root cells, reduces nutrient uptake capacity, and creates conditions favourable to pythium and other root pathogens. This is why growers who focus only on shading the canopy while ignoring root-zone cooling see limited improvement.</p>

<h2>Irrigation: The Central Challenge</h2>
<p>In arid Australian conditions, irrigation is not supplementary — it is the primary growing input. The plant's water demand in dry, hot air exceeds what manual watering schedules can reliably deliver. Drip irrigation systems with timers outperform hand watering because they maintain consistent root-zone moisture without the boom-bust cycle of infrequent heavy watering followed by rapid dry-out.</p>
<p>Mulching the soil surface reduces evaporation by 30-40% and is non-negotiable in dry-climate grows. Straw, wood chips, or landscape fabric create a barrier between the soil surface and dry air. Without mulch, the top 5 cm of medium desiccate within hours, which stresses surface-level feeder roots and wastes water.</p>
<p>Water quality matters more in arid regions. Municipal water in many inland Australian towns has higher dissolved mineral content (hard water with elevated calcium and magnesium), which affects pH buffering and nutrient interactions. Test your source water and adjust your feeding program accordingly. Rainwater collection, where practical, provides a softer alternative.</p>

<h2>Container Strategy and Root-Zone Cooling</h2>
<p><strong>Container colour:</strong> Black plastic pots absorb radiant heat and cook roots. Light-coloured containers reflect heat. Fabric pots (tan or white) allow evaporative cooling from the pot walls and run 5-8°C cooler than dark plastic in direct sun.</p>
<p><strong>Container insulation:</strong> Burying containers 50-75% in the ground leverages earth's thermal mass — soil temperature 30 cm below surface stays relatively stable even when air temperatures swing 20°C between day and night. For above-ground containers, wrapping in reflective insulation or placing inside larger containers with an air gap provides insulation from radiant heat.</p>
<p><strong>Ground vs. container:</strong> In-ground planting gives roots access to cooler subsurface soil and virtually unlimited root space. The tradeoff: you lose the ability to move plants and drainage depends entirely on native soil composition. In heavy clay or rocky Australian soils, raised beds with imported potting mix may outperform direct ground planting.</p>

<h2>Wind Desiccation and Sun Management</h2>
<p>Hot, dry Australian winds strip moisture from leaf surfaces faster than roots can replace it, producing a chronic transpiration deficit that mimics underwatering even when the root zone is wet. Windbreaks — fencing, companion planting, shade cloth, or site selection behind structures — are essential, not optional.</p>
<p>Shade cloth (30-40% reduction) during peak afternoon hours is counterintuitive for most growers but frequently improves results in Australian heat. Full summer sun at noon delivers 2000+ PPFD — well beyond cannabis's saturation point of 800-1000 PPFD. The excess light becomes excess heat load with no photosynthetic benefit. Partial shade reduces heat stress while maintaining adequate light for productive growth.</p>

<h2>Cultivar Selection for Hot, Dry Australian Conditions</h2>
<p>Traits that matter for success in arid Australia: vigorous root development, efficient water use, heat-tolerant leaf structure, pest resistance (dry climates reduce mould but increase spider mite pressure), and a flowering period that finishes before the worst late-summer heat or that can be timed to avoid it.</p>
<p>${CL.sat} from tropical and equatorial lineages often handle heat better than dense ${CL.ind} because their leaf structure facilitates more efficient transpiration. However, pure sativas need 10-14 weeks of flower, which may span the worst heat months (January-February). The pragmatic approach: ${CL.auto} planted in early spring (September-October) to harvest before December heat peaks, or planted in late February to finish during the milder April-May window. ${CL.fast} genetics offer a similar advantage for photoperiod growers who want to minimise heat exposure during the most vulnerable flowering weeks.</p>

<h2>Arid-Climate Mistakes</h2>
<p><strong>Underestimating water demand:</strong> A plant in 38°C dry air can drink 4+ litres per day in a 20-litre container. Growers coming from temperate coastal climates are routinely shocked by how fast inland plants dehydrate.</p>
...
<p><strong>Growing genetics bred for cool European climates:</strong> Dense-budded indicas bred for Dutch greenhouses struggle in Australian heat. Choose genetics with proven heat tolerance or landrace tropical heritage.</p>

<h2>Frequently Asked Questions</h2>
<dl>
<dt>Can I grow cannabis outdoors in inland Australia?</dt>
<dd>Yes, but timing matters. Early spring (September-November) and autumn (March-May) are the most viable outdoor windows. Mid-summer heat above 40°C makes productive outdoor growing extremely difficult without significant infrastructure (shade structures, misting, automated irrigation).</dd>
<dt>Is indoor growing easier in dry Australian climates?</dt>
<dd>In some ways, yes. Low ambient humidity means dehumidifiers are unnecessary and mould risk is minimal. But cooling the grow space can be expensive during summer when outdoor temperatures exceed 40°C — air conditioning costs are a real factor in inland Australian indoor grows.</dd>
<dt>What is the biggest mistake dry-climate growers make?</dt>
<dd>Choosing strains based on potency or popularity rather than heat tolerance. A 28% THC genetic that dies in the heat produces zero harvest. A 20% THC strain that thrives in your conditions produces excellent flower. Match genetics to environment first.</dd>
</dl>
`;

  if (isAlt) return `
<p>Growing cannabis at elevation in Australia — the Blue Mountains, Snowy Mountains, Victorian Highlands, and Tasmanian ranges — introduces three variables that fundamentally change cultivation: UV intensity increases roughly 10% per 1,000 metres of elevation gain, nighttime temperatures drop sharply (15-20°C below daytime highs), and the growing season is compressed by earlier autumn frost. These are not minor tweaks — they reshape which genetics succeed, how you feed and water, and what the final flower looks like compared to the same strain grown at sea level.</p>

<h2>UV Intensity: The Double-Edged Advantage</h2>
<p>Higher UV intensity at altitude drives increased trichome production. Cannabis produces trichomes partly as UV protection — a biological sunscreen that happens to contain the cannabinoids and terpenes growers value. This is why some of the most resinous, terpene-rich outdoor cannabis comes from elevated growing sites.</p>
...
<p>Genetics matter here. Strains with heritage from high-altitude regions (Hindu Kush genetics, certain South American landraces) handle UV better than equatorial lowland genetics that evolved under filtered tropical light.</p>

<h2>Nighttime Temperature Drops: What Actually Happens</h2>
<p>At altitude, nighttime lows commonly drop 15-20°C below daytime highs. Cannabis tolerates cool nights down to about 10°C without major growth impact, and actually benefits from moderate DIF (day/night temperature differential): cool nights slow transpiration, improve terpene retention (volatile terpenes evaporate faster in warm air), and can trigger anthocyanin production in genetically predisposed cultivars — which is why ${CL.purple} grown at altitude develop more intense colouration than the same genetics grown in warm lowland conditions.</p>
<p>Below 7°C, metabolic processes slow significantly — root uptake drops, growth stalls, and flowering extends beyond the expected timeframe. Below 2°C, cellular damage begins. Frost (0°C) kills exposed flower tissue — ice crystals form inside cells and rupture cell walls, destroying bud structure and releasing chlorophyll that turns the flower brown and harsh.</p>

<h2>Season Length: The Constraining Factor</h2>
<p>Most high-altitude Australian locations — the Blue Mountains, Victorian Alps, Tasmanian highlands — offer outdoor growing windows from late October to mid-March, roughly 16-18 weeks. Subtract 4 weeks for seedling establishment, and you have about 12-14 weeks for veg and flower. This eliminates any cultivar with a flowering period exceeding 8-9 weeks unless you start plants indoors and transplant out after last frost.</p>
<p>${CL.auto} are the safest choice for altitude: they flower on their own schedule regardless of day length and finish in 10-12 weeks from seed. ${CL.fast} offer the next best option — finishing 1-2 weeks ahead of standard photoperiod strains. For growers committed to photoperiod genetics, beginning veg indoors under lights before transplanting outdoors reduces the calendar time the plant needs in the elements.</p>
...
<p><strong>Planting too early:</strong> Mountain spring comes later than valley spring. A warm week in September followed by a hard frost in October kills transplants that went out too early. Wait until consistent nighttime lows stay above 7°C.</p>
<p><strong>Choosing long-flowering sativas:</strong> An 11-week sativa started outdoors in November needs to flower through mid-April at altitude — well past first frost in most mountain locations. Unless you have season-extension infrastructure (greenhouse, poly tunnel), stick to 8-9 week strains maximum.</p>
...
<dd>Only if the genetics carry anthocyanin potential. Cool nights trigger the colour expression, but the genes must be present. Not every strain purples at altitude — but those that do often develop more vivid colouration than the same genetics grown in warm conditions.</dd>
<dt>Can I grow indoors at altitude year-round?</dt>
<dd>Yes, and indoor growing at altitude benefits from naturally low humidity (less mould risk) and cool ambient temperatures (less cooling cost). The main consideration is that some indoor growers at altitude report faster soil drying due to lower atmospheric pressure.</dd>
</dl>
`;

  if (isCold) return `
<p>Cannabis tolerates cold better than most growers expect, but there are hard limits — and the consequences of hitting them range from extended flowering time to total crop loss. For Australian growers in Tasmania, the Victorian Highlands, the ACT, and elevated regions of NSW, understanding exactly what cold does to the plant, where the thresholds are, and how to select genetics and timing that work within a short season is what separates successful cold-climate growers from frustrated ones.</p>

<h2>What Cold Actually Does to Cannabis</h2>
<p><strong>10-15°C:</strong> Growth slows but continues. Terpene production and retention may actually benefit from cool nights (volatile terpenes evaporate less in cool air). Anthocyanin expression increases in genetically predisposed strains. This range is uncomfortable but not damaging.</p>
<p><strong>7-10°C:</strong> Root uptake slows significantly. Nutrients in the soil are available but roots process them sluggishly, producing deficiency-like symptoms that are actually cold-related. Plants in this range often show purple stems, slow growth, and pale new leaves — not because nutrients are missing, but because roots cannot move them at this temperature.</p>
<p><strong>2-7°C:</strong> Growth effectively stops. Metabolic processes are too slow for productive photosynthesis. Flowering extends well beyond expected timelines. Prolonged exposure at this range does not kill the plant but makes harvesting before further cold becomes a race against weather.</p>
<p><strong>Below 0°C (frost):</strong> Ice crystals form inside plant cells, rupturing cell walls. Damage is irreversible. Affected tissue turns brown, goes limp, and develops an unpleasant taste if harvested. A single hard frost can destroy weeks of flower development overnight.</p>

<h2>Root-Zone Temperature: The Hidden Variable</h2>
<p>Air temperature and root-zone temperature can differ significantly — and roots are more cold-sensitive than foliage. A plant whose leaves tolerate a 9°C night may have roots in 10°C soil that are struggling to uptake nutrients and water. This disconnect produces symptoms that look like nutrient deficiency but do not respond to feeding — because the issue is temperature, not chemistry.</p>
<p>Solutions: elevate containers off cold ground (even 15 cm on a wooden pallet makes a difference), use insulated or double-walled containers, apply thick mulch over the soil surface, or water with slightly warm (18-21°C) water during cold snaps to moderate root-zone temperature.</p>

<h2>Cultivar Selection for Short, Cold Australian Seasons</h2>
<p><strong>${CL.auto}:</strong> The safest choice for cold climates. Their flowering is age-triggered, not light-triggered, so they do not waste weeks waiting for day length to shorten in autumn. An autoflower planted in late November in Tasmania, the ACT, or southern Victoria can be harvested by mid-to-late February — well before first frost risk. This timing buffer makes autoflowers nearly foolproof for short-season outdoor growing.</p>
<p><strong>${CL.fast}:</strong> Photoperiod genetics bred to finish 1-2 weeks earlier than standard strains. These provide more yield potential than autoflowers (because you control veg time) with a reduced frost-risk window. For growers with 5-month outdoor seasons, fast-flowering feminised strains are the performance-optimised choice.</p>
...
<p>Simple poly tunnels or greenhouse covers extend the season by 2-4 weeks by trapping daytime heat and buffering nighttime temperature drops. Even a clear plastic sheet draped over a support frame raises the microclimate temperature enough to protect against light frost (-2 to 0°C) and keep nighttime lows above the 7°C growth threshold.</p>
<p>Row covers (frost blankets) provide 2-4°C of protection on cold nights and can be deployed and removed quickly. Some cold-climate growers in southern Australia use fairy lights under row covers for radiant heat on the coldest nights — a low-tech solution that provides just enough warmth to prevent frost damage.</p>

<h2>Cold-Climate Mistakes</h2>
<p><strong>Planting too early:</strong> A warm October week followed by a hard November frost kills transplants. Wait until consistent overnight temperatures stay above 7°C before moving plants outdoors — typically mid-November in most cool-climate Australian locations.</p>
<p><strong>Growing long-season strains:</strong> An 11-week photoperiod sativa in southern Victoria needs to flower into May. That is not going to happen before frost. Match flowering time to your actual frost-free window, not your optimistic assessment of autumn weather.</p>
...
<h2>Frequently Asked Questions</h2>
<dl>
<dt>What is the latest I can plant outdoors in a cold Australian climate?</dt>
<dd>For autoflowers: mid-December is typically the latest to ensure harvest before March-April frost. For photoperiod strains: they need to be outdoors and established by early November to have enough veg time before natural day-length changes trigger flowering in late summer.</dd>
<dt>Can cannabis survive a light frost?</dt>
<dd>A brief, light frost (-1 to 0°C for a few hours) may damage exposed leaf edges and uppermost flower tissue while lower, insulated parts of the plant survive. But any frost damages quality, and repeated frost exposure degrades the harvest rapidly. Prevention is always better than tolerance.</dd>
<dt>Is it worth growing outdoors if my season is only 4 months?</dt>
<dd>With autoflowers, yes — they can complete a full lifecycle in 10-12 weeks, which fits into a November-February window. With photoperiod strains, a 4-month season is marginal. Indoor growing or a season-extension structure (greenhouse, poly tunnel) would significantly improve your results.</dd>
</dl>
`;

  if (isHumid) return `
<p>In humid climates above 60% relative humidity during late flower, the interior of dense cannabis buds creates a microenvironment where moisture cannot escape, condensation forms between calyxes, and fungal spores germinate. Botrytis (bud rot) and powdery mildew do not need much to take hold — just moisture, stagnant air, and a few days of favourable conditions. For growers in coastal Queensland, northern NSW, tropical Darwin, or anywhere along Australia's east coast that stays above 60% RH through harvest season, humidity management is not a variable to optimise — it is a threat to manage.</p>
...
<p>This is not a failure of growing technique. It is a physics problem: dense flower in humid air cannot maintain the air exchange needed to prevent internal condensation. Growers who produce beautiful dense buds indoors and try the same genetics outdoors in Brisbane or Cairns learn this the hard way.</p>

<h2>Bud Rot (Botrytis): Identification and Response</h2>
<p>Botrytis typically starts inside the densest top colas — exactly where you most want it not to be. Early signs: a single brown, dead leaf poking out of an otherwise healthy bud. When you pull that leaf, the interior reveals grey-brown mould threading through the flower structure. By the time you see it, the rot has been developing for 3-5 days internally.</p>
<p>Response: remove the affected bud immediately, cutting 2-5 cm below the visible rot into clean tissue. Inspect all dense top colas on the plant. If multiple colas are affected, consider an early harvest of the entire plant to salvage unaffected flower — the rot will continue spreading as long as conditions permit.</p>
...
<p>PM thrives in conditions of high humidity combined with moderate temperatures (15-27°C) and poor air circulation. It spreads through airborne spores that land on wet or humid leaf surfaces. Prevention: keep RH below 55% in the canopy, ensure oscillating airflow across all leaf surfaces, and remove any infected material immediately to prevent spore distribution.</p>

<h2>Strain Selection for Humid Australian Climates</h2>
<p>The traits that resist humidity: <strong>open, airy bud structure</strong> that allows air movement through the flower (the opposite of what most strain descriptions advertise as desirable); <strong>natural airflow through the canopy</strong> from moderate leaf density rather than excessive foliage; <strong>shorter flowering periods</strong> that finish before autumn humidity peaks; and <strong>genetic resistance</strong> from tropical or equatorial landrace lines that evolved in humid environments over thousands of generations.</p>
<p>${CL.sat} from equatorial regions often have the loosest bud structure and best natural mould resistance. Their tradeoff: long flowering times (10-14 weeks) that may push harvest into the worst humidity months. The practical compromise for humid-climate Australian growers: sativa-dominant ${CL.hyb} that combine tropical genetics with shorter flowering periods (9-10 weeks instead of 12-14). This gives you the open structure and genetic resistance of sativa heritage with a timeline that allows harvest before conditions deteriorate.</p>
<p>${CL.auto} planted in early spring (September-October) to harvest before summer humidity peaks offer another viable strategy. In Queensland, a September-planted autoflower can be harvested by late November or early December — before the wettest part of the wet season arrives.</p>

<h2>Environmental Management for Humid-Climate Grows</h2>
<p><strong>Airflow:</strong> The single most important controllable variable. Oscillating fans that move air through the canopy (not just above it) prevent the stagnant microenvironments where moisture accumulates. Outdoor growers can improve airflow by spacing plants further apart, removing lower foliage that blocks air movement, and strategic plant positioning relative to prevailing sea breezes.</p>
...
<p><strong>Harvest timing:</strong> In humid Australian climates, a slightly early harvest (mostly milky trichomes, minimal amber) may produce better final quality than waiting for maximum maturity if the extra days mean exposing flower to conditions that promote rot. A clean, healthy harvest at 90% maturity beats a mouldy harvest at 100% maturity.</p>

<h2>Frequently Asked Questions</h2>
<dl>
<dt>Can I grow dense indica strains in humid Australian climates?</dt>
...
<dd>No, but outdoor growing requires deliberate strain selection and timing. Sativa-dominant hybrids with open bud structure, planted to finish before peak humidity season, produce successful outdoor harvests in humid Australian climates. Greenhouses with ventilation offer a middle ground.</dd>
...
<p>Your environment determines: how fast the plant grows, when it can flower, how much water it needs, what pests and pathogens it faces, which terpenes express most strongly, and ultimately, what the flower looks and smokes like at harvest. The same seed in Queensland full sun and a Melbourne garage produces noticeably different plants — not because genetics changed, but because the environment shaped every aspect of growth and expression.</p>
...
<h2>Australian Climate Zone Strain Matching</h2>
<p><strong>Short seasons (under 5 months frost-free — Tasmania, ACT, Victorian Highlands):</strong> ${CL.auto} or ${CL.fast}. Non-negotiable for outdoor growing. The calendar does not negotiate.</p>
<p><strong>Hot, dry climates (inland SA, WA, western NSW):</strong> Heat-tolerant genetics, preferably with tropical or equatorial heritage. Irrigation infrastructure is essential. Container management to prevent root-zone overheating. Consider shade cloth during peak afternoon heat.</p>
<p><strong>Hot, humid climates (coastal QLD, northern NSW, Top End):</strong> Mould-resistant cultivars with open bud structure. Sativa-dominant hybrids. Timing that avoids peak wet-season months. Airflow management is critical.</p>
<p><strong>Cool, wet climates (Tasmania, southern Victoria):</strong> Fast-finishing genetics with mould resistance. Indoor growing provides the most reliable results. Outdoor growing is viable with careful timing and strain selection.</p>
<p><strong>Indoor (any Australian climate):</strong> Full genetic range is available. Environment is controlled. Focus on matching genetics to your space dimensions (compact plants for short tents, trained plants for larger spaces) and your management style (autoflowers for simplicity, photoperiods for control).</p>

<h2>The Decision That Matters Most</h2>
<p>Before buying seeds, honestly assess your environment: what is your growing season? What are your temperature extremes? What is your typical humidity range during the months you will be flowering? How much space do you have? What equipment do you have or can you get? The answers narrow your options in a useful way. The ${CL.all} can be filtered by growing conditions, flowering time, and difficulty — use those filters before browsing by potency or flavour, and your success rate will improve immediately.</p>

<h2>Frequently Asked Questions</h2>
<dl>
<dt>What if my climate does not match any ideal growing condition?</dt>
<dd>Most Australian locations can produce quality cannabis — the key is matching genetics to your specific constraints rather than fighting them. Indoor growing removes climate as a variable entirely. Outdoor growing in challenging climates requires more careful strain selection but is achievable with the right genetics and timing.</dd>
<dt>Should I grow indoors or outdoors?</dt>
<dd>Indoor gives control and year-round growing. Outdoor gives larger plants, natural light, and lower equipment cost. Many Australian growers do both — indoor for consistency, outdoor for volume during the long summer season. Your best choice depends on your space, budget, climate zone, and legal situation.</dd>
...
function stateGuideArticle(slug: string, title: string): string {
  ...
  const strains = pickN(slug, [
    {n:'Gorilla Glue',h:'/gorilla-glue-cannabis-seeds',note:'Heavy resin production with potent full-body effects. Dense flower structure and straightforward growing behaviour. One of our most consistent performers across Australian climates.'},
    ...
    {n:'Granddaddy Purple',h:'/granddaddy-purple-cannabis-seeds',note:'Grape-forward terpene profile with heavy indica relaxation. Compact plant that finishes quickly and develops striking purple colouration in cool nighttime temperatures.'},
    {n:'Northern Lights',h:'/northern-lights-cannabis-seeds',note:'The definition of beginner-friendly — low odour, fast flower, resilient to environmental stress, and forgiving of feeding mistakes. Excellent first-grow genetics.'},
    {n:'Blue Dream',h:'/product-category/best-seller',note:'Sativa-leaning hybrid with high yield potential. Balanced effects that work for daytime and evening. One of our highest-selling strains across all Australian states.'},
    ...
    {n:'Cheese',h:'/cheese-cannabis-seeds',note:'Distinctive savoury aroma unlike anything else in cannabis. Indica-leaning effects with a unique funk that distinguishes it from every other strain.'},
    {n:'SFV OG',h:'/sfv-og-cannabis-seeds',note:'San Fernando Valley genetics with heavy indica character. Dense, frosty buds and a sedative effect profile favoured by evening users and pain management.'},
  ], 10);

  return `
<p>Growing cannabis in ${state} comes with its own set of conditions, regulations, and climate realities that generic growing guides do not address. This guide draws from order patterns and grow reports from our ${state} customer base — what strains they choose, what challenges they report, and what produces the most consistent results in this region specifically.</p>

<h2>Legal Context for ${state} Home Cultivators</h2>
<p>Cannabis seeds are sold as adult novelty souvenirs and for genetic preservation purposes. Home cultivation laws in ${state} are set at the state or territory level and can change between legislative sessions. Before germinating any seeds, verify your current rights under ${state} law through your state or territory's official cannabis regulatory body or government website. Our ${a('FAQ page', '/faq')} covers the general federal and state framework, but local verification is your responsibility. We do not provide legal advice — only factual information about our products and shipping.</p>

<h2>Recommended Cannabis Seeds for ${state}</h2>
<p>These recommendations are based on what our ${state} customers actually grow successfully — not just what sells best nationally. Each strain below has a demonstrated track record with growers in this region's specific conditions.</p>
...
<p>Indoor cultivation gives ${state} growers year-round growing capability regardless of outdoor conditions. The standard setup — a 1 m × 1 m or 1.2 m × 1.2 m grow tent, 200W+ LED light, exhaust fan with carbon filter, and basic environmental monitoring — covers the essential equipment for a productive indoor grow.</p>
...
<p>Common indoor challenges in ${state}: ${pick(slug, [
    'humidity management during the warmer months when ambient humidity rises, increasing the risk of mould in late flower. A dehumidifier is recommended for grows that overlap with the humid season.',
    'temperature control during extreme weather — cooling in summer and heating in winter can be the primary ongoing cost. Insulating the grow space from ambient temperature swings reduces HVAC demand.',
    'odour management in attached housing. A quality carbon filter handles odour for most home grows, but the filter should be sized appropriately for the exhaust fan CFM rating to work effectively.',
  ], state)}</p>

<h2>Outdoor Growing Considerations for ${state}</h2>
<p>Outdoor success in ${state} depends on your specific microclimate — elevation, proximity to the coast, urban heat island effects, prevailing wind patterns, and first/last frost dates all vary within the state. The general approach for the Southern Hemisphere: match your strain's flowering period to your frost-free window (typically October through April), adjust for local humidity and temperature patterns, and choose genetics with traits suited to your specific conditions.</p>
<p>For outdoor growing in ${state} with a season shorter than 5 months, ${CL.auto} or ${CL.fast} are the most reliable options — they complete their lifecycle independently of day length and finish before cold weather threatens the harvest. Longer outdoor seasons in northern and coastal areas open up ${CL.fem} with higher yield potential. ${P.harvest(state)}</p>

<h2>Climate-Specific Tips for ${state}</h2>
<p>${pick(slug, [
    `Growers in the southern and inland parts of ${state} typically face shorter seasons and cooler temperatures than coastal regions. Autoflowers or fast-flowering genetics planted after last frost provide the most reliable outdoor results. Indoor growing eliminates this constraint entirely.`,
    ...
    `The temperature range in ${state} favours indoor growing for year-round production. Outdoor growing is viable during the warmer months (October-April) with appropriate genetics. Our ${CL.outdoor} collection includes strains evaluated for resilience in variable conditions.`,
    `${state} growers benefit from the growing community in the region — local knowledge about microclimates, planting dates, and strain performance supplements the general guidance in this guide.`,
  ], state + 'climate')}</p>
...
<p>All orders ship discreetly to ${state} addresses via Australia Post with tracking on every package. See our ${a('shipping page', '/shipping')} for delivery time estimates and packaging details.</p>

<h2>Frequently Asked Questions</h2>
<dl>
<dt>What is the best strain for a first-time grower in ${state}?</dt>
<dd>Northern Lights Auto is our most-recommended first-grow strain for ${state} — forgiving, low odour, fast finish, and resilient to the mistakes every new grower makes. If you want a photoperiod option, Northern Lights (non-auto) is equally forgiving with higher yield potential.</dd>
<dt>When should I plant outdoors in ${state}?</dt>
<dd>After your local last frost date, when consistent nighttime temperatures stay above 10°C. For autoflowers, this is typically mid-October to mid-November in most ${state} locations. Photoperiod plants benefit from being started indoors 2-3 weeks before transplanting outdoors.</dd>
<dt>Do you ship to ${state}?</dt>
<dd>Yes. We ship to all ${state} addresses via Australia Post with tracking. All orders ship in plain, unmarked packaging with no external indication of contents. See our ${a('shipping page', '/shipping')} for details.</dd>
</dl>
`;
}
...
function terpeneArticle(slug: string, title: string): string {
  ...
  return `
<p>Terpenes are the reason two strains at 22% THC can feel completely different — and the reason why reducing cannabis to a THC percentage misses most of what makes a specific cultivar worth growing. These volatile aromatic compounds shape flavour, modulate how cannabinoids interact with the endocannabinoid system, and contribute to the subjective "character" of each strain in ways that researchers are still mapping but that growers and users have observed for decades.</p>

<h2>What Terpenes Are and Why They Matter Beyond Smell</h2>
<p>Terpenes are organic compounds produced in the trichome glands alongside cannabinoids. Cannabis produces over 200 identified terpenes, though most strains are dominated by 2-5 primary terpenes that define their aromatic and experiential character. These compounds evolved as the plant's defence system — deterring herbivores, attracting pollinators, and protecting against UV radiation. Their interaction with human physiology is a secondary effect that happens to produce the nuanced, strain-specific experiences cannabis is valued for.</p>
...
<p><strong>Light intensity and spectrum:</strong> Higher light intensity (up to the plant's saturation point) drives more trichome production, which means more terpene production. UV-B light specifically has been shown to increase terpene synthesis — this is why outdoor cannabis grown at altitude (higher UV) and indoor grows with UV-supplemented LED often produce stronger aromatic profiles. Australian outdoor grows benefit from naturally high UV levels, particularly during the height of summer.</p>
<p><strong>Temperature differential:</strong> Moderate day/night temperature differences (5-8°C) promote terpene retention. Cool nights slow the evaporation of volatile terpenes. This is why many growers drop their nighttime temperature in the final 2 weeks of flower — a practical technique that often produces noticeable aromatic improvement. Autumn harvests in southern Australia naturally provide this cool-night advantage.</p>
...
<dt>Does vaporising preserve more terpenes than smoking?</dt>
<dd>Yes. Combustion temperatures (230°C+) destroy most terpenes. Vaporisation at 170-195°C selectively releases terpenes at their individual boiling points, preserving more of the aromatic and experiential profile. This is particularly noticeable with terpene-rich ${CL.exotic} and ${CL.fruity} strains.</dd>
</dl>
`;
}
...
<p>Cannabinoid production is primarily determined by genetics, but environmental factors modify the final ratio. Light intensity and spectrum influence total cannabinoid production — higher light drives more trichome development, which means more total cannabinoid content. Australian outdoor grows benefit from intense natural sunlight, particularly during the December-February peak. Nutrient management affects plant health and by extension trichome quality (stressed plants produce less than healthy ones). And harvest timing directly shifts the cannabinoid ratio: early harvest preserves maximum THC relative to CBN, while late harvest increases CBN content as THC degrades. ${P.harvest('')}</p>
<p>This is why the same strain from different grows can test differently — the genetics set the potential, but the growing environment and harvest decision determine what percentage of that potential is realised.</p>
...
<h2>Testing and Labelling: What to Trust and What to Question</h2>
...
<p>For users who wake during the night, edibles provide longer-lasting effects (6-8 hours) compared to inhalation (2-3 hours). A low-dose edible (5-10 mg THC from a sedative indica) taken 90 minutes before bed can provide sustained overnight coverage that inhalation cannot. ${P.anxiety('')}</p>

<h2>Strain Recommendations for Sleep Support</h2>
<p><strong>Heavy indicas with myrcene dominance:</strong> ${CL.kush} varieties (Bubba Kush, OG Kush, Hindu Kush, Purple Kush) are the most commonly cited sleep strains among our Australian customers. Their earthy, musky terpene profiles and strong physical effects align with what sleep-focused users seek.</p>
...
<p>Based on conversations with our Australian customer base, users distinguish between two broad categories of cannabis-assisted pain management:</p>
...
<p><strong>Edibles:</strong> Slower onset (30-90 minutes), longest duration (4-8 hours). Preferred by users who need sustained coverage — overnight pain management, all-day comfort during flare-ups. The slower onset makes dose titration harder, so starting with a low dose (5-10 mg) and waiting 2 hours before considering more is critical.</p>
...
<dd>Start with the minimum effective dose. For inhalation, one small hit of a known sedative strain. For edibles, 5 mg THC. Wait for full effect before considering more. The effective dose for pain management is often lower than the dose needed for strong recreational effects.</dd>
...
<p><strong>Stimulating terpenes:</strong> Terpinolene in high concentrations can feel energising to the point of restlessness in anxiety-prone individuals. High limonene can feel "buzzy" or mentally activating. Pinene promotes alertness, which is helpful for focus but can feel like agitation if anxiety is already elevated.</p>
<p>The practical takeaway: anxiety-prone users should prioritise strains with linalool, myrcene, or caryophyllene dominance and be cautious with terpinolene-heavy or high-limonene genetics. ${P.terp('')}</p>
...
<p>From that baseline, users who want to explore mild THC effects can move to balanced 1:1 THC/CBD strains — the CBD component moderates THC's psychoactive intensity and reduces the probability of anxiety escalation. Our ${a('anxiety-relief strain collection', '/product-category/best-strains-for-anxiety')} is organised from lowest THC to highest THC to support this progressive approach.</p>

<h2>Dosing Strategy for Anxiety</h2>
<p><strong>Start low:</strong> 1 small inhalation, or 2.5-5 mg THC if using edibles. Wait 15-20 minutes (inhalation) or 90 minutes (edibles) before considering more. The effective dose for anxiety relief is almost always lower than the dose that produces strong recreational effects.</p>
...
<p>Based on feedback from thousands of Australian customers who specifically select strains for anxiety management:</p>
...
<p>THCA converts to THC through heat (thermal decarboxylation). The optimal balance between complete conversion and minimal degradation occurs at 115°C for 40-60 minutes. Higher temperatures speed conversion but also degrade THC into CBN and destroy terpenes — which matters if you want flavour in your edible, not just potency. Lower temperatures preserve more terpenes but require longer exposure (107°C for 90 minutes is the slow-and-gentle alternative).</p>
<p>The process: coarsely grind your flower (do not powder it — fine grinding exposes more surface area to heat and increases degradation). Spread evenly on parchment-lined baking tray. Bake at 115°C for 40-50 minutes. The flower should darken slightly from green to brownish-green and smell toasty. Let cool before handling.</p>
<p>Alternative: sous vide decarboxylation. Seal ground flower in a vacuum bag, submerge in 95°C water bath for 90 minutes. This method provides more precise temperature control and eliminates the odour that oven decarboxylation produces — a relevant advantage for growers in shared living spaces.</p>
...
<p><strong>Coconut oil:</strong> The preferred carrier for most home cooks. High saturated fat content (82%) binds cannabinoids efficiently. Neutral flavour at moderate doses. Solid at room temperature, which makes it easy to handle and store. Substitutes directly for butter or oil in most recipes.</p>
...
<p>Infusion process: combine decarboxylated flower with chosen fat at 7-10 grams per 250 mL (1 cup). Maintain 70-82°C for 2-4 hours — a slow cooker on low, a double boiler, or a sous vide circulator at 80°C all work. Stir occasionally. Strain through cheesecloth or fine mesh, pressing gently to extract infused oil without pushing plant material through. The resulting infusion stores in the refrigerator for 4-6 weeks or in the freezer for months.</p>

<h2>Dosing: The Maths and the Mistakes</h2>
...
<p>Example: 7 grams (7,000 mg) of 20% THC flower × 0.20 × 0.88 = ~1,232 mg THC in the total infusion. If this is in 250 mL of oil and you make 28 cookies, each cookie contains ~44 mg THC — well above a reasonable single dose for most people.</p>
...
<p><strong>For flavour:</strong> ${CL.fruity} and ${CL.exotic} add noticeable flavour character to infusions. Limonene-dominant strains produce a citrusy undertone. Myrcene-dominant strains produce an earthier base. The flavour difference is most noticeable in simple recipes (cannabutter on toast, infused honey) and less noticeable in heavily flavoured dishes.</p>
...
<dd>Infused oil/butter: 4-6 weeks refrigerated, 6+ months frozen. Baked goods: same as non-infused versions (biscuits: 1 week at room temp, 2-3 months frozen). Gummies/lollies: several weeks at room temperature in airtight containers.</dd>
...
<dd>Two causes: too much plant material relative to fat, and insufficient straining. Using less flower per cup of oil (5-7 g instead of 10+), water-curing decarboxylated flower before infusion (soaking in water for 24 hours to leach chlorophyll), and straining through multiple layers of cheesecloth all reduce the vegetal taste without reducing potency.</dd>
...
<p>${pick(slug, [
    `The temperature at which cannabis is consumed is the single biggest variable in what compounds you actually experience. Combustion (above 230°C) destroys a significant percentage of terpenes and converts some cannabinoids into less desirable pyrolytic byproducts. Vaporisation at 170-195°C selectively releases cannabinoids and terpenes at their individual boiling points — preserving far more of the aromatic and experiential complexity that the genetics and growing effort produced.`,
    `The preparation step between jar and consumption has more impact on the final experience than most users realise. A dull grinder crushes trichomes instead of separating them cleanly, releasing sticky resin onto metal surfaces instead of into the prepared material. An overly fine grind increases surface area exposed to combustion, producing faster, hotter burning that destroys more compounds. A medium grind that maintains some structural integrity produces more even vaporisation or combustion.`,
    `For users who chose their genetics specifically for terpene character — ${CL.fruity} for citrus, ${CL.kush} for earthy depth, ${CL.exotic} for complex profiles — the consumption method is the final bottleneck. Combustion eliminates most of what makes those genetics distinctive. Low-temperature vaporisation preserves it. The difference in flavour between smoking and vaping the same flower at 190°C is dramatic enough that many growers who switch to vaporisation say they "never knew what their flower actually tasted like" until they stopped burning it.`,
  ], 'core')}</p>

<h2>${topic}: Practical Considerations</h2>
<p>${pick(slug, [
    `Quality tools make a measurable difference — not because expensive always means better, but because poorly designed or maintained equipment actively degrades the product. A glass piece that has not been cleaned in weeks adds residue flavour that obscures the terpene expression you spent months cultivating. A metal grinder with teeth that have gone dull crushes rather than cuts, destroying trichome glands and wasting the most valuable part of the flower. Clean equipment, sharp edges, and appropriate temperatures are the three controllable variables that preserve quality.`,
    ...
    `Temperature precision has become accessible through modern vaporisers with digital controls. This matters because different terpenes vaporise at different temperatures: myrcene at 167°C, limonene at 176°C, caryophyllene at 130°C, linalool at 198°C. By adjusting temperature, you can selectively emphasise different aspects of the terpene profile — lower temperatures for brighter, more volatile aromatics; higher temperatures for heavier, more sedative compounds. This is not theoretical; it is immediately perceptible to any user who experiments with temperature settings across a 170-205°C range.`,
  ], 'prac')}</p>

<h2>Storage: Protecting What You Grew</h2>
<p>Post-cure storage priorities: darkness, cool temperature (15-21°C), stable humidity (58-62% RH), and minimal air exposure. Glass mason jars remain the gold standard for home storage — inert material, airtight seal, no static charge that strips trichomes (unlike plastic). UV-blocking glass or storage in a dark cupboard prevents light degradation of THC to CBN.</p>
...
<p>The growers who report the best final experiences are the ones who care about every stage, not just the grow room. Growing from seed gives you control over the entire chain that no other source can match.</p>
...
<dd>Glass pieces: after every 3-5 uses for optimal flavour. Grinders: monthly, or when residue buildup is visible. Vaporisers: follow manufacturer guidelines, but generally the heating chamber should be brushed after each session and deep-cleaned weekly. Clean equipment preserves the flavour profile you worked months to develop.</dd>
<dt>Is there a noticeable quality difference between cheap and expensive vaporisers?</dt>
<dd>Yes, primarily in temperature accuracy and vapour path cleanliness. Budget vaporisers often have poor temperature calibration (actual temperature varies from the display) and use materials in the vapour path that can off-gas at operating temperatures. Mid-range to quality devices provide accurate, consistent temperatures and medical-grade vapour path materials that do not interfere with flavour.</dd>
</dl>
`;
}
...
function legalArticle(slug: string, title: string): string {
  ...
  return `
<p>Cannabis regulation continues to evolve faster than any other area of drug policy worldwide. What was prohibited last year may be decriminalised now. What is legal in one jurisdiction may carry penalties a state border away. For growers and consumers making purchasing, cultivation, and travel decisions, understanding the current landscape — and its practical implications — requires more than headlines.</p>

<h2>Current Regulatory Context: ${topic}</h2>
<p>${pick(slug, [
    `${topic} represents part of the broader global trend toward cannabis regulation reform. The specific regulatory approach varies significantly — some jurisdictions have embraced full legalisation for adult use, others have implemented medical-only frameworks, and some maintain prohibition while decriminalising possession. The details matter: legal purchase does not always mean legal cultivation, and legal cultivation does not always mean unlimited plant counts.`,
...
<h2>What This Means for Seed Buyers and Home Growers in Australia</h2>
<p>Cannabis seeds are sold as adult novelty souvenirs and for genetic preservation purposes. The legality of germination and cultivation depends entirely on your specific jurisdiction — not on where the seeds were purchased. In Australia, cannabis law is determined at both federal and state/territory level, and the Australian Capital Territory has moved furthest with personal use legislation. The broader global trend is toward expanded home cultivation rights: more countries and jurisdictions are legalising, decriminalising, or creating medical access pathways than restricting them.</p>
<p>For growers considering purchasing seeds: the act of buying seeds is distinct from the act of germinating them in most legal frameworks. Verify your local laws through official government sources. Our ${a('FAQ page', '/faq')} covers the general legal framework as it relates to seed purchasing in Australia, and our ${a('shipping page', '/shipping')} confirms that we deliver Australia-wide with discreet packaging via Australia Post.</p>

<h2>How Legalisation Affects Seed Quality and Availability</h2>
<p>One positive consequence of expanding legalisation globally: the quality and diversity of cannabis genetics available to home growers has improved dramatically. As legal markets grow, professional breeders invest more in genetic stability, germination testing, and novel strain development. The seeds available today — from established breeders in the Netherlands, Spain, North America, and beyond — represent a significantly higher quality standard than what was available even five years ago.</p>
<p>At Royal King Seeds, we source from over 40 breeders and maintain germination testing on incoming batches. The global expansion of the cannabis industry has directly improved our ability to source diverse, stable, high-quality genetics for our ${CL.all}.</p>

<h2>The Economic Reality: Why Home Growing Continues to Grow in Popularity</h2>
<p>Even in jurisdictions with legal medical markets, cannabis prices remain high — often $15-20 AUD per gram for quality flower through medical channels. Home growing from a single ${CL.fem} seed ($5-10 AUD per seed) can produce hundreds of grams of flower from one plant, making the cost comparison dramatically favourable. Combined with full control over genetics, growing practices, and harvest timing, home cultivation offers both economic and quality advantages that legal market access does not eliminate.</p>
<p>This economic reality is a significant driver behind the home growing movement in Australia and explains why seed sales continue to grow even as medical cannabis access expands. Access to information, equipment, and quality genetics removes the barriers that previously kept interested growers from starting.</p>

<h2>Staying Informed</h2>
<p>Cannabis law is not static. We recommend checking your jurisdiction's official government cannabis regulatory website at least once per year, especially before starting a new grow. Legislative sessions, regulatory rulemaking, and policy changes can shift the landscape between growing seasons. Our ${a('blog', '/blog')} covers significant regulatory developments as they affect Australian home growers, but official government sources should always be your primary legal reference.</p>

<h2>Frequently Asked Questions</h2>
<dl>
<dt>Are cannabis seeds legal to buy in Australia?</dt>
...
<dt>Does the ACT's personal use law affect other states?</dt>
<dd>No. Cannabis law in Australia is determined at the state and territory level. The ACT's personal cultivation provisions apply only within the ACT. Each state and territory's regulations are independent. What is permitted in the ACT does not extend legal protection to other jurisdictions.</dd>
<dt>How do I find current laws for my specific area?</dt>
<dd>Search for your state or territory's official health department or cannabis regulatory information. The Australian Government's health department website also provides federal-level information. Always verify through official government sources rather than relying solely on third-party summaries.</dd>
</dl>
`;
}
...
<p>${pick(slug, [
    ...
    `${topic} touches cannabis in ways that are easy to dismiss as peripheral until you realise the connection runs deeper than it appears. For growers, consumers, and anyone making informed choices about cannabis, the context behind topics like this informs better decisions.`,
  ], 'open')}</p>
